...
from ingestion_airflow.config import LogicalCdcReplayRunConfig
...
from ingestion_core.strategies.logical_cdc import apply_wal_delta_to_curated
...
def _replay_input_error(problem: str, details: list[str]) -> AirflowFailException:
    message = "\n".join(
        [
            "Invalid logical CDC replay input.",
            f"Problem: {problem}",
            *details,
        ]
    )
    payload = {
        "error": "invalid_logical_cdc_replay_input",
        "problem": problem,
        "details": details,
        "message": message,
    }
    logger.error("INVALID_LOGICAL_CDC_REPLAY_INPUT\n%s", message)
    try:
    ...
    return AirflowFailException(
        "Invalid logical CDC replay input. "
        "See INVALID_LOGICAL_CDC_REPLAY_INPUT in task log or XCom key replay_input_error."
    )


@dag(
    dag_id="replay_contract_logical_cdc_from_minio",
    start_date=datetime(2026, 1, 1),
    schedule=None,
    catchup=False,
    max_active_runs=1,
    tags=["ingestion", "logical-cdc", "pgoutput", "replay", "minio"],
)
def replay_contract_logical_cdc_from_minio() -> None:
    def _load_run_config() -> LogicalCdcReplayRunConfig:
        context = get_current_context()
        dag_run = context["dag_run"]
        return LogicalCdcReplayRunConfig.from_dagrun_conf(dag_run.conf or {})
    ...
    def resolve_replay_input() -> dict[str, Any]:
        ...
        if stage_metrics:
            ...
            if not delta_object_key:
                raise _replay_input_error(
                    "Parent logical CDC run has extract metadata without delta_object_key.",
                    [
                        f"parent_run_id: {config.parent_run_id}",
                        "Expected: stage_audit.metrics_json.delta_object_key must point to accepted WAL delta.",
                        "Fix: use a successful ingest_contract_logical_cdc run_id as parent_run_id.",
                    ],
                ) from None
        else:
            artifacts = build_logical_cdc_artifacts(config.namespace, config.name, config.parent_run_id)
            delta_object_key = artifacts["delta_object_key"]
            validation_manifest_key = artifacts["validation_manifest_key"]

        if not delta_object_key:
            raise _replay_input_error(
                "Replay WAL delta key could not be resolved.",
                [
                    "Expected: parent_run_id references a successful ingest_contract_logical_cdc run.",
        ...
        if contract_version is None and validation_manifest_key:
            if not object_store.object_exists(validation_manifest_key):
                ...
                expected_artifacts = build_logical_cdc_artifacts(
                    config.namespace,
                    config.name,
                    config.parent_run_id,
                )
                ...
                raise _replay_input_error(
                    "Logical CDC replay manifest was not found in object store.",
                    [
                        f"parent_run_id: {config.parent_run_id}",
                        "Expected: parent_run_id must be a successful ingest_contract_logical_cdc run_id "
                        "from ingestion_meta.run_audit, not an Airflow DagRun id and not another strategy run.",
                        f"Successful WAL extract stage metadata found in audit DB: {parent_stage_metrics_found}",
        ...
        if not object_store.object_exists(delta_object_key):
            ...
            raise _replay_input_error(
                "Logical CDC replay delta artifact was not found in object store.",
                [
...
dag_instance = replay_contract_logical_cdc_from_minio()
