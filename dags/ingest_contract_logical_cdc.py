...
def ingest_contract_logical_cdc() -> None:
    ...
    def _execute_stage(
        ...
        try:
            ...
            logger.info("Stage %s succeeded for run_id=%s result=%s", stage_name, run_id, metrics_payload)
            finish_stage_audit(audit_engine, run_id, stage_name, "success", metrics_payload, None)
            ...
            finish_stage_audit(audit_engine, run_id, stage_name, "failed", failure_payload, str(exc))
            raise
        finally:
                ...
                **ensure_source_logical_cdc_capture(
                    source_admin_dsn=with_application_name(
                        ...
                        build_application_name(
                            strategy=STRATEGY,
                            run_mode=RUN_MODE,
                            stage_name="ensure_source_logical_cdc_capture",
                            role="source-admin",
                        ),
                    ),
            ...
            lambda: extract_validate_land_wal_delta(
                source_dsn=with_application_name(
                    ...
                    build_application_name(
                        strategy=STRATEGY,
                        run_mode=RUN_MODE,
                        stage_name="extract_validate_land_wal_delta",
                        role="source",
                    ),
                ),
                source_replication_dsn=with_application_name(
                    config.source_replication_dsn,
                    build_application_name(
                        strategy=STRATEGY,
                        run_mode=RUN_MODE,
                        stage_name="extract_validate_land_wal_delta",
            ...
            lambda: ack_logical_replication_slot(
                source_replication_dsn=with_application_name(
                    ...
                    build_application_name(
                        strategy=STRATEGY,
                        run_mode=RUN_MODE,
                        stage_name="ack_replication_slot",
                        role="replication",
                    ),
                ),
                source_slot_name=config.source_slot_name,
    ...
    def finalize_run(run_context: dict[str, Any]) -> dict[str, bool]:
        ...
        extract_stage_record = read_stage_audit_record(
            audit_engine,
        ...
        try:
            if failed_tasks:
                finish_run_audit(
                    ...
                    str(run_context["run_id"]),
                    "failed",
                    int(extract_stage_payload.get("source_event_count", 0) or 0),
                    int(apply_result.get("insert_count", 0) or 0),
                    int(apply_result.get("update_count", 0) or 0),
                    int(apply_result.get("delete_count", 0) or 0),
            ...
            run_metrics = enrich_metrics_payload(
                payload={
                    **apply_result,
                    ...
                    "invalid_transaction_count": extract_stage_payload.get("invalid_transaction_count"),
                    "quarantined_event_count": extract_stage_payload.get("quarantined_event_count"),
                    "quarantined_transaction_count": extract_stage_payload.get("quarantined_transaction_count"),
                    "validation_error_object_key": (
                        extract_stage_payload.get("validation_error_object_key")
                        or extract_stage_payload.get("error_object_key")
                    ),
                    ...
                    "window_start_lsn": extract_stage_payload.get("window_start_lsn"),
                    "window_end_lsn": extract_stage_payload.get("window_end_lsn"),
                    "last_decoded_lsn": extract_stage_payload.get("last_decoded_lsn"),
                    "last_stream_wal_end_lsn": extract_stage_payload.get("last_stream_wal_end_lsn"),
                    "reached_window_end": extract_stage_payload.get("reached_window_end"),
                },
                strategy=STRATEGY,
                run_mode=RUN_MODE,
                pipeline_id=config.pipeline_id,
